...
def publish(output,tag,title,notes):
    identity=json.loads((output/'release-identity.json').read_text())
    ...
    assets=json.loads(subprocess.check_output([GH,'api',f'repos/{REPO}/releases/{release["id"]}/assets?per_page=100'],text=True))
    remote={a['name']:a for a in assets}
    ...
    write_json(output/'remote-verification.json',{'release_url':release['html_url'],'assets':checked,'status':'completed'})
...
if __name__=='__main__':
    ...
    p.add_argument('--publish',action='store_true');p.add_argument('--tag');p.add_argument('--title');p.add_argument('--notes',type=Path)
    a=p.parse_args()
